...
def approach(self, edges):
    ...
    acc_window = (0, min(cfg.q_a, (cfg.q_v - (quadr_orient.project(self.speed).length))/(cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))))
    w_window = (-min(cfg.q_angle_accel, (cfg.q_angle_vel - self.angle_speed)/(cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))), min(cfg.q_angle_accel, (cfg.q_angle_vel - self.angle_speed)/(cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))))
    
    ch_table = []

    for w in np.arange(w_window[0], w_window[1], cfg.appr_w_res):
        for acc in np.arange(acc_window[0], acc_window[1], cfg.appr_a_res):
            pred_pos, pred_vel = predict_cords(self.att_rope, Vector3D(np.sin(self.angle), np.cos(self.angle+w/2*(cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))), 0)*acc)
    ...
    self.a_forced = best_sp[0] * Vector3D(np.sin(self.angle+best_sp[1]/2*(cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))), np.cos(self.angle+best_sp[1]/2*(cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))), 0)
...
def predict_cords(self, forced_a):
    ...
    for i in range(n):
        ...
        speed += F/s.m * (cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))
        pred_vel[i] = speed

        pos += speed * (cfg.sk / cfg.fps / int(cfg.upfr / cfg.fps))
        pred_pos[i] = pos
